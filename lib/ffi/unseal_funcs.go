...
func (sb *SealCalls) DecodeSnap(ctx context.Context, taskID harmonytask.TaskID, commD, commK cid.Cid, sector storiface.SectorRef) error {
	...
	sealReader, err := sb.sectors.storage.ReaderSeq(ctx, sector, storiface.FTUpdate)
	if err != nil {
		return xerrors.Errorf("getting unsealed sector reader: %w", err)
	}
	...
	err = cunative.DecodeSnap(sector.ProofType, commD, commK, sealReader, keyReader, outFile)
	if err != nil {
		return xerrors.Errorf("decoding sealed sector: %w", err)
	}
