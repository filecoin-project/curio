...
//! round-trip, MSM dispatch grid, **SRS `h[]` / `b_g2[0]` decode** ([`srs`]) + G1 **full-width Pippenger** MSM ([`split_msm::g1_msm_pippenger_gpu_batch_shared_bases`] — §8.4 **D.2** shared `h[]`), and GPU **H** vs CPU
...
//! **§C.1 slice:** [`VulkanDevice`] `VkPipelineCache` + optional **`CUZK_VK_PIPELINE_CACHE`**. **B₂ C.2:** [`srs_staging_gpu`] device-local upload (`vkCmdCopyBuffer`, no readback) + async submit / [`SrsDeviceLocalUploadGuard`](crate::srs_staging_gpu::SrsDeviceLocalUploadGuard) overlapped with Fr NTT in [`prove_groth16_partition`](crate::prover::prove_groth16_partition).
...
pub mod subgroup_shuffle_smoke_gpu;
pub mod toy_ntt;
pub mod toy_ntt_gpu;
...
pub use device_profile::{
    msm_config_for_device, msm_config_for_pippenger_gpu, recommended_msm_window_bits,
};
