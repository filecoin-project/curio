...
func (p *PDPService) handleGetPieceStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpServerError(w, "Failed to query piece status", err)
		return
	}
...
func (p *PDPService) handleGetDataSetCreationStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpServerError(w, "Failed to query data set creation", err)
		return
	}
	...
	if err != nil {
		...
		httpServerError(w, "Failed to query message status", err)
		return
	}
	...
	if dataSetCreate.DataSetCreated {
		...
		if err != nil {
			...
			httpServerError(w, "Failed to query data set", err)
			return
		}
...
func (p *PDPService) handleGetDataSet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpServerError(w, "Failed to retrieve data set", err)
		return
	}
    ...
    `, dataSetId)
	if err != nil {
		httpServerError(w, "Failed to retrieve data set pieces", err)
		return
	}
	...
	if err != nil {
		httpServerError(w, "Failed to retrieve next challenge epoch", err)
		return
	}
...
func (p *PDPService) handleGetPieceAdditionStatus(w http.ResponseWriter, r *http.Request) {
	...
	`, dataSetId, txHash)
	if err != nil {
		httpServerError(w, "Failed to query piece additions", err)
		return
	}
	...
	if err != nil {
		...
		httpServerError(w, "Failed to query transaction status", err)
		return
	}
	...
	if txStatus == "confirmed" && len(pieceAdds) > 0 && pieceAdds[0].AddMessageOK != nil && *pieceAdds[0].AddMessageOK {
		...
		`, dataSetId, txHash)
		if err != nil {
			httpServerError(w, "Failed to query confirmed pieces", err)
			return
		}
	}
...
func (p *PDPService) handleDeleteDataSetPiece(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpServerError(w, "Failed to query piece existence", err)
		return
	}
	if !found {
	...
	if err != nil {
		httpServerError(w, "Failed to get sender address", err)
		return
	}
	...
	if err != nil {
		httpServerError(w, "Failed to send transaction", err)
		return
	}
	...
	if err != nil {
		httpServerError(w, "Failed to schedule delete piece", err)
		return
	}
...
func (p *PDPService) handleGetDataSetPiece(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpServerError(w, "Failed to retrieve piece", err)
		return
	}
	...
	`, dataSetId, pieceID)
	if err != nil {
		httpServerError(w, "Failed to retrieve subPieces", err)
		return
	}
