...
func StartTasks(ctx context.Context, dependencies *deps.Deps, shutdownChan chan struct{}) (*harmonytask.TaskEngine, error) {
...

	{
		...
		if cfg.Subsystems.EnablePDP {
			...
			pdpPullPieceTask := pdpv0.NewPDPPullPieceTask(ctx, db, lstor, cfg.Subsystems.PDPPullPieceMaxTasks)
...
func addSealingTasks(
	...
	if cfg.Subsystems.EnableMoveStorage {
		...
		storePieceTask, err := piece2.NewStorePieceTask(db, must.One(slrLazy.Val()), stor, cfg.Subsystems.StorePieceMaxTasks)
		if err != nil {
			return nil, nil, err
		}
