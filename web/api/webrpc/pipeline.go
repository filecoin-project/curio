...
func (a *WebRPC) PipelinePorepSectors(ctx context.Context) ([]sectorListEntry, error) {
	...
	stor := store.ActorStore(context.Background(), blockstore.NewReadCachedBlockstore(blockstore.NewAPIBlockstore(a.deps.Chain), curiochain.ChainBlockCache))
	...
	for _, task := range tasks {
		...
		if !ok {
			mbf, err = a.getMinerBitfields(ctx, addr, stor)
			if err != nil {
