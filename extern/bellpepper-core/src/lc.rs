...
impl<T> Indexer<T> {
    /// Create an `Indexer` wrapping a pre-allocated (but empty) buffer.
    /// The buffer should have been cleared before passing in.
    #[inline]
    fn from_recycled(buf: Vec<(usize, T)>) -> Self {
        debug_assert!(buf.is_empty(), "recycled buffer must be cleared");
        Indexer {
            values: buf,
            last_inserted: None,
        }
    ...
    /// Consume this `Indexer` and return the inner buffer (cleared).
    /// The returned Vec retains its heap allocation for reuse.
    #[inline]
    fn recycle(mut self) -> Vec<(usize, T)> {
        self.values.clear();
        self.values
    }
}
...
impl<Scalar: PrimeField> LinearCombination<Scalar> {
    ...
    /// Create a zero `LinearCombination` using pre-allocated, cleared buffers.
    ///
    /// This avoids heap allocation on the hot path. The caller must provide
    /// two empty Vec buffers (one for inputs, one for aux) that retain their
    /// heap capacity from a previous use.
    #[inline]
    pub fn zero_recycled(
        inputs_buf: Vec<(usize, Scalar)>,
        aux_buf: Vec<(usize, Scalar)>,
    ) -> LinearCombination<Scalar> {
        LinearCombination {
            inputs: Indexer::from_recycled(inputs_buf),
            aux: Indexer::from_recycled(aux_buf),
        }
    }

    /// Consume this `LinearCombination` and return its inner buffers (cleared)
    /// for reuse. Each returned Vec retains its heap allocation.
    #[inline]
    pub fn recycle(self) -> (Vec<(usize, Scalar)>, Vec<(usize, Scalar)>) {
        (self.inputs.recycle(), self.aux.recycle())
    }
    ...
    /// Like `from_coeff`, but takes pre-allocated buffers for the Indexers.
    /// The buffer for the side not receiving the variable is used as-is (empty),
    /// while the other side gets a single element pushed.
    #[inline]
    pub fn from_coeff_recycled(
        var: Variable,
        coeff: Scalar,
        mut inputs_buf: Vec<(usize, Scalar)>,
        mut aux_buf: Vec<(usize, Scalar)>,
    ) -> Self {
        debug_assert!(inputs_buf.is_empty() && aux_buf.is_empty());
        match var {
            Variable(Index::Input(i)) => {
                inputs_buf.push((i, coeff));
                Self {
                    inputs: Indexer {
                        values: inputs_buf,
                        last_inserted: Some((0, i)),
                    },
                    aux: Indexer::from_recycled(aux_buf),
                }
            }
            Variable(Index::Aux(i)) => {
                aux_buf.push((i, coeff));
                Self {
                    inputs: Indexer::from_recycled(inputs_buf),
                    aux: Indexer {
                        values: aux_buf,
    ...
    /// Direct slice access to input terms for prefetch-friendly iteration.
    #[inline]
    pub fn input_terms_slice(&self) -> &[(usize, Scalar)] {
        &self.inputs.values
    }

    /// Direct slice access to aux terms for prefetch-friendly iteration.
    #[inline]
    pub fn aux_terms_slice(&self) -> &[(usize, Scalar)] {
        &self.aux.values
    }
    ...
    /// Prefetch a cache line into L1 data cache (x86_64 only).
    #[inline(always)]
    fn prefetch_read(ptr: *const Scalar) {
    ...
    pub fn eval(&self, input_assignment: &[Scalar], aux_assignment: &[Scalar]) -> Scalar {
        ...
        let inp = self.input_terms_slice();
        for (i, (index, coeff)) in inp.iter().enumerate() {
            if i + 1 < inp.len() {
                Self::prefetch_read(unsafe { input_assignment.as_ptr().add(inp[i + 1].0) });
            }
        ...
        let aux = self.aux_terms_slice();
        for (i, (index, coeff)) in aux.iter().enumerate() {
            if i + 1 < aux.len() {
                Self::prefetch_read(unsafe { aux_assignment.as_ptr().add(aux[i + 1].0) });
            }
