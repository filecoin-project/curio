...
func (p *ProveTask) proveRoot(ctx context.Context, dataSetID int64, pieceID int64, challengedLeaf int64) (contract.IPDPTypesProof, error) {
	...
	err := p.db.QueryRow(ctx, `
		SELECT pr.piece_cid, pr.id, pp.piece_raw_size
		FROM pdp_data_set_pieces dsp
		JOIN pdp_piecerefs pr ON pr.id = dsp.pdp_pieceref
		JOIN parked_piece_refs pprf ON pprf.ref_id = pr.piece_ref
		JOIN parked_pieces pp ON pp.id = pprf.piece_id
		WHERE dsp.data_set = $1 AND dsp.piece_id = $2`, dataSetID, pieceID).Scan(&pieceCidStr, &piecerefID, &rawSize)
	if err != nil {
		return contract.IPDPTypesProof{}, xerrors.Errorf("failed to get piece cid for dataset piece: %w", err)
	}

	pcidV1, err := cid.Parse(pieceCidStr)
	if err != nil {
		return contract.IPDPTypesProof{}, xerrors.Errorf("failed to parse piece CID: %w", err)
	}

	pcidV2, err := commcid.PieceCidV2FromV1(pcidV1, rawSize)
	if err != nil {
		return contract.IPDPTypesProof{}, xerrors.Errorf("failed to construct piece cid v2: %w", err)
	}
	...
	if rawSize < MinSizeForCache {
		// Get original file reader
		reader, _, err := p.cpr.GetSharedPieceReader(ctx, pcidV1)
		if err != nil {
		...
		memTree, err := proof.BuildSha254Memtree(reader, padreader.PaddedSize(rawSize).Padded().Unpadded())
		if err != nil {
		...
		if !has {
			log.Errorf("No proving cache found for piece %s. Create a save cache task", pcidV2.String())
			err = p.startSaveCache(ctx, piecerefID)
			if err != nil {
				return contract.IPDPTypesProof{}, xerrors.Errorf("failed to start save cache task: %w", err)
			}
			return contract.IPDPTypesProof{}, xerrors.Errorf("No proving cache found for piece %s. Create a save cache task", pcidV2.String())
		}
		...
		if !has {
			log.Errorf("Proving cache does not have the node for piece %s. Create a save cache task", pcidV2.String())
			err = p.startSaveCache(ctx, piecerefID)
			if err != nil {
				return contract.IPDPTypesProof{}, xerrors.Errorf("failed to start save cache task: %w", err)
			}
			return contract.IPDPTypesProof{}, xerrors.Errorf("Proving cache does not have the node for piece %s. Create a save cache task", pcidV2.String())
		}
		...
		// Get original file reader
		reader, reportedSize, err := p.cpr.GetSharedPieceReader(ctx, pcidV1)
		if err != nil {
		...
		com, _, err := commcid.PieceCidV2ToDataCommitment(pcidV2)
		if err != nil {
...
func (p *ProveTask) startSaveCache(ctx context.Context, piecerefID int64) error {
	_, err := p.db.Exec(ctx, `UPDATE pdp_piecerefs SET needs_save_cache = TRUE, save_cache_task_id = NULL WHERE id = $1`, piecerefID)
	if err != nil {
		return xerrors.Errorf("failed to trigger save cache: %w", err)
	}
	return nil
}
