...
class MarketAsks extends LitElement {
    ...
    async loadData() {
        ...
        // Filter and map addresses to spIDs
        const spIDs = addresses
            .map((addr) => parseInt(addr.substring(2)));
    ...
    async saveAsk() {
        ...
        try {
            await RPCCall('SetStorageAsk', [this.newAsk]);
            // Reload data
            await this.loadData();
            // Close the form
            this.updatingSpID = null;
            ...
            <form @submit="${this.handleSubmit}">
              <div class="modal-header">
                <h5 class="modal-title">
                  Set Storage Ask for SP ${this.updatingSpID}
                </h5>
                <button
                  type="button"
                  class="btn-close"
              ...
              <div class="modal-body">
                <div class="mb-3">
                  <label class="form-label">Price (attoFIL per GiB/epoch)</label>
                  <input
                    type="number"
                    class="form-control"
                    .value="${this.newAsk.Price}"
                    @input="${(e) => (this.newAsk.Price = e.target.value)}"
                    required
                  />
                </div>
                <div class="mb-3">
                  <label class="form-label"
                    >Verified Price (attoFIL per GiB/epoch)</label
                  >
                  <input
                    type="number"
                    class="form-control"
                    .value="${this.newAsk.VerifiedPrice}"
                    @input="${(e) => (this.newAsk.VerifiedPrice = e.target.value)}"
                    required
                  />
                </div>
                <div class="mb-3">
