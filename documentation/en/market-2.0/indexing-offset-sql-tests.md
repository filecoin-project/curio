...
These executable SQL assertions cover the NULL-offset readiness fix.
Execution requires a separately authorized disposable database. Record the
source revision, database version, baseline results, and negative-control
results separately for each engine; compilation is not SQL execution.
...
Do not commit/push either mutation or disable the safety guard. A successful
negative control establishes SQL row effects on the tested engine, distinct
from a database-free SQL-shape mutation. It does not establish an impossible
interleaving within the single atomic assignment statement.

Existing assigned tasks, exhausted retries, post-GC offset discovery, stored
schedules, and completion recovery remain outside this fix and verification.
