...
func TestClusterTaskSnapshotSQL(t *testing.T) {
	...
	empty, err := source.LoadSnapshot(ctx, ClusterTaskSummaryApplied{MaxTasks: 500, MaxPending: 500})
	require.NoError(t, err)
	require.Empty(t, empty.Rows)
	require.Zero(t, empty.RunningTotal)
	require.Zero(t, empty.PendingTotal)
...
SELECT n, CURRENT_TIMESTAMP-INTERVAL '3 hours', CASE WHEN n<=3 THEN 101 ELSE NULL END,101,'Synthetic' FROM generate_series(1,503) n`)
	...
	// Test the final generic argument order and cap allocation, not the former
	// personal priority query. Counts intentionally include rows outside the page.
	for _, tc := range []struct{ max, pending, rows int }{{2, 500, 2}, {5, 0, 3}, {500, 500, 500}} {
		got, err := source.LoadSnapshot(ctx, ClusterTaskSummaryApplied{MaxTasks: tc.max, MaxPending: tc.pending})
		require.NoError(t, err)
		require.Len(t, got.Rows, tc.rows)
		require.Equal(t, int64(500), got.PendingTotal)
	}
	_, err = db.Exec(ctx, `INSERT INTO harmony_task (id,posted_time,owner_id,added_by,name) VALUES (504,CURRENT_TIMESTAMP,101,101,'bg:synthetic'),(505,CURRENT_TIMESTAMP,NULL,101,'Other')`)
	require.NoError(t, err)
	name := "Other"
	filtered, err := source.LoadSnapshot(ctx, ClusterTaskSummaryApplied{MaxTasks: 500, MaxPending: 500, TaskName: &name})
	require.NoError(t, err)
	require.Len(t, filtered.Rows, 1)
	require.Equal(t, int64(505), filtered.Rows[0].ID)
	require.Nil(t, filtered.Rows[0].OwnerID)
	withBackground, err := source.LoadSnapshot(ctx, ClusterTaskSummaryApplied{MaxTasks: 500, MaxPending: 500, IncludeBackground: true})
	require.NoError(t, err)
	require.Equal(t, int64(4), withBackground.RunningTotal)
	withoutBackground, err := source.LoadSnapshot(ctx, ClusterTaskSummaryApplied{MaxTasks: 500, MaxPending: 500})
	require.NoError(t, err)
	require.Equal(t, int64(3), withoutBackground.RunningTotal)
	_, err = db.Exec(ctx, `UPDATE harmony_task SET owner_id=NULL`)
	require.NoError(t, err)
	pending, err := source.LoadSnapshot(ctx, ClusterTaskSummaryApplied{MaxTasks: CLUSTER_TASK_SUMMARY_DEFAULT_MAX_TASKS, MaxPending: CLUSTER_TASK_SUMMARY_DEFAULT_MAX_PENDING})
	require.NoError(t, err)
	require.Len(t, pending.Rows, 500, "default pending preview must not retain the personal 30-row policy")
	require.Equal(t, int64(504), pending.PendingTotal)
