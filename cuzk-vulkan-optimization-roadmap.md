...
| B.5 | Coset first-stage fusion — **[1.05×, S, pack-fused `base^i` + forward NTT landed (`run_fr_ntt_general_forward_gpu_distribute`); optional SPV+butterfly merge still open, risk: low]** |
...
| 6 | **Done (B₀ + B₁ slice)** | [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs): Fr NTT round-trip + MSM dispatch grid; with `CUZK_VK_SKIP_SMOKE=0`, **SRS-bound** G1 MSM ([`srs_decode_h_g1`](extern/cuzk/cuzk-vk/src/srs.rs) + [`split_msm::g1_msm_pippenger_gpu`](extern/cuzk/cuzk-vk/src/split_msm.rs) — **255-bit** window bucket accumulate vs `multi_exp`) and **H** GPU vs CPU ([`h_term_gpu`](extern/cuzk/cuzk-vk/src/h_term_gpu.rs)). SRS H₂D overlaps Fr NTT when dual compute queues exist ([`SrsDeviceLocalUploadGuard`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs)). **B₂ parity backlog:** [`MILESTONE_B.md`](extern/cuzk/MILESTONE_B.md) (Vulkan-native proof, MSM perf parity levers, witness path, …). |
...
5. **General Fr NTT** — SSBO twiddle bases, multi-dispatch (`vk_oneshot` pass sequence); **coset forward** on GPU ([`fr_coset_gpu`](extern/cuzk/cuzk-vk/src/fr_coset_gpu.rs), `tests/fr_coset_fft_gpu.rs`) through `n = 2^14`, **radix-8 forward + pack-fused distribute when `n ≥ 8`**. **CPU** coset: `tests/fr_coset_fft_cpu.rs`. **B.5 partial:** pack fusion + radix-8 coset leg; optional single-SPV coset+first butterfly remains open. **Partition smoke** [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs): radix-8 **round-trip** when `n ≥ 8`.
...
7. **Split MSM orchestration** — bitmap + GPU batch + CPU tail Pippenger. **Milestone A (slice done):** [`split_msm`](extern/cuzk/cuzk-vk/src/split_msm.rs) G1 bit-plane MSM + **CPU** cross-check vs `G1Projective::multi_exp` (`tests/split_msm_multiexp_cpu.rs`). **Milestone B₂:** [`g1_msm_bitplanes_scalars_trunc_gpu_host`](extern/cuzk/cuzk-vk/src/split_msm.rs) + `tests/split_msm_scalar_trunc_gpu.rs` (small `n`, truncated scalars). **Landed (full-width):** [`g1_msm_pippenger_gpu`](extern/cuzk/cuzk-vk/src/split_msm.rs) → [`run_g1_pippenger_msm_gpu`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) + `tests/g1_pippenger_full_width_gpu.rs`; partition smoke uses Pippenger on SRS `h[]`. **Still open:** CUDA-scale density-aware split (A.5), GPU window assembly / fewer submits, §8.4 **performance** mega-strip + **D.2** SRS sharing (multi-circuit **correctness** batched path: [`run_g1_pippenger_msm_gpu_batch`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs)).
8. **SRS** — mmap layout from `groth16_srs.cuh`, synthetic fixture tests. **Slice done:** [`srs`](extern/cuzk/cuzk-vk/src/srs.rs) VK + IC decode + `b_g2[]`, `srs_read_file`, **[`srs_read_file_spawn`](extern/cuzk/cuzk-vk/src/srs.rs)** (B₁ disk-read overlap), `tests/srs_file_ic.rs` / `tests/srs_async_read.rs`; [`srs_gpu`](extern/cuzk/cuzk-vk/src/srs_gpu.rs) G1 `g1_reverse24` + **G2** `g2_reverse48` Montgomery limb staging vs CPU (`tests/srs_g1_gpu_reverse24.rs`, `tests/srs_g2_gpu_reverse48.rs`). **Milestone B₂:** mmap + **GPU** upload overlap vs first NTT — **partition smoke:** [`SrsDeviceLocalUploadGuard`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) in [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs); **remainder:** production consumes device-local SRS.
9. **H-term pipeline** — INTT/FFT/coset chain vs CPU reference. **Done (Milestone A):** [`h_term`](extern/cuzk/cuzk-vk/src/h_term.rs) `fr_quotient_scalars_from_abc`, [`h_term_gpu`](extern/cuzk/cuzk-vk/src/h_term_gpu.rs) uses [`fr_coset_gpu`](extern/cuzk/cuzk-vk/src/fr_coset_gpu.rs) for coset forward NTT (§8.2 **B.5** pack-fused distribute) and GPU tail distribute (`g^{-1}`); host vanishing `z_inv` between GPU stages; tests `h_term_quotient_gpu` (incl. domain 8192). **Milestone B:** optional single-SPV coset+first butterfly fusion.
10. **`prove_groth16_partition`** — real driver + Groth16 verify test. **B₀:** partition ties Fr NTT + MSM grid + SRS `h[]` MSM + GPU H ([`prover`](extern/cuzk/cuzk-vk/src/prover.rs)) when `CUZK_VK_SKIP_SMOKE=0`; **native** Groth16 prove+verify: [`groth16_verify_tiny`](extern/cuzk/cuzk-vk/tests/groth16_verify_tiny.rs). **B₁:** [`milestone_b_bellperson_vulkan_smoke`](extern/cuzk/cuzk-vk/tests/milestone_b_bellperson_vulkan_smoke.rs). **B₂:** MSM §8.1 perf parity + Vulkan-emitted proof from bellperson witness + pairing on that proof — see [`MILESTONE_B.md`](extern/cuzk/MILESTONE_B.md).
...
| 2 | **§8.1 MSM (A.*)** | **Correctness landed:** [`run_g1_pippenger_msm_gpu`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) (GPU buckets + host Horner), tests to `n=1024`. **Open:** A.2–A.7 perf levers, GPU window assembly / submit batching, mega-dispatch §8.4 (see **§8.7** MSM row). |
| 3 | **§8.2 NTT (B.*)** | GPU radix-4/8, subgroup stages, **B.5 pack-fused coset distribute + forward NTT** ([`run_fr_ntt_general_forward_gpu_distribute`](extern/cuzk/cuzk-vk/src/fr_ntt_general_gpu.rs)), single-shot small-*n* (**B.6**). |
| 4 | **§8.3 C.2** | **Landed:** [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs) overlaps SRS `vkCmdCopyBuffer` submit with Fr NTT ([`SrsDeviceLocalUploadGuard`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs), [`queue_compute_1`](extern/cuzk/cuzk-vk/src/device.rs)). **Open:** consume device-local SRS in downstream kernels instead of host-only decode replay. |
...
| 6 | **§8.4 D.*** | **Partial:** [`run_g1_pippenger_msm_gpu_batch`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) + tiling vs shader budgets; [`msm_mega_dense_groups_x`](extern/cuzk/cuzk-vk/src/msm.rs). **Open:** mega-strip perf shader + **D.2** cross-circuit SRS window sharing. |
...
**B₂ work queue (not “one PR”):** §8.1 **correctness landed:** [`run_g1_pippenger_msm_gpu`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) + [`g1_msm_pippenger_gpu`](extern/cuzk/cuzk-vk/src/split_msm.rs) (`tests/g1_pippenger_full_width_gpu.rs`); partition smoke uses Pippenger vs `multi_exp`. **Perf remainder:** A.2–A.7, GPU window combine, fewer submits, §8.4; [`device_profile::msm_config_for_device`](extern/cuzk/cuzk-vk/src/device_profile.rs) + **`CUZK_VK_MSM_WINDOW_BITS`**. **Small-n demo:** [`g1_msm_bitplanes_scalars_trunc_gpu_host`](extern/cuzk/cuzk-vk/src/split_msm.rs) + [`scalar_low_u64_canonical`](extern/cuzk/cuzk-vk/src/scalar_limbs.rs) (`tests/split_msm_scalar_trunc_gpu.rs`). §8.2 **partial:** [`FrNttPlan::fused_layer_count`](extern/cuzk/cuzk-vk/src/ntt.rs) for radix-4/8 **schedule** preview (GPU radix butterflies still open). §8.3 **C.2 landed (partition smoke):** [`srs_staging_device_local_upload_submit_async`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) + [`SrsDeviceLocalUploadGuard`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) in [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs) — **remainder:** production path consumes SRS from GPU memory vs host decode-only replay today. **C.3 partial:** [`pipelines::spec_constant_u32_le`](extern/cuzk/cuzk-vk/src/pipelines.rs) + [`vk_oneshot::run_compute_1x_storage_buffer`](extern/cuzk/cuzk-vk/src/vk_oneshot.rs) optional [`ComputeShaderStageSpec`](extern/cuzk/cuzk-vk/src/vk_oneshot.rs) + multi-pass `ComputePassDesc::stage_spec`; smoke [`run_spec_constant_smoke_gpu`](extern/cuzk/cuzk-vk/src/spec_constant_smoke_gpu.rs); MSM grid smoke [`run_msm_dispatch_hitcount_smoke`](extern/cuzk/cuzk-vk/src/msm_gpu.rs) (`local_size_x_id`, `msm_dispatch_grid_smoke_prebuilt.spv`). **Remainder:** Fr NTT tails; MSM bucket **`layout(constant_id)`** where **naga** cannot emit spec IDs (see §8.3 C.3). §8.4 **partial:** batched Pippenger [`run_g1_pippenger_msm_gpu_batch`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) + [`pippenger_plan_batch_tiles`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs); strip sketch [`msm_mega_dense_groups_x`](extern/cuzk/cuzk-vk/src/msm.rs). **Perf remainder:** mega-strip shader + D.2 SRS window sharing + submit batching. §1 **partial:** [`VkProofTimings`](extern/cuzk/cuzk-vk/src/prover.rs) per-stage wall + **`CUZK_VK_BENCH_CSV`** → [`bench_csv`](extern/cuzk/cuzk-vk/src/bench_csv.rs); hardware matrix + threshold asserts still open.
...
|----|--------|---------------|--------|
| A.1 | Per-vendor MSM window auto-tune | ~1.10× | **Slice:** [`device_profile`](extern/cuzk/cuzk-vk/src/device_profile.rs) + **`CUZK_VK_MSM_WINDOW_BITS`** wired into [`run_g1_pippenger_msm_gpu`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) / partition smoke. **Open:** VALU-driven tuning vs static hints. |
...
| B.5 | Coset first-stage fusion | ~1.05× | **Partial:** [`run_fr_ntt_general_forward_gpu_distribute`](extern/cuzk/cuzk-vk/src/fr_ntt_general_gpu.rs) — `base^i` in SSBO pack removes GPU pointwise dispatch before NTT. **Open:** single SPV merging coset twiddle with first radix butterfly. |
...
| C.2 | Async SRS upload vs first NTT | 0.5–3 s hidden | **B₁:** [`srs_read_file_spawn`](extern/cuzk/cuzk-vk/src/srs.rs) overlaps **disk** read with host work. **B₂:** [`srs_staging_device_local_upload_submit_async`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) + [`SrsDeviceLocalUploadGuard`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) on [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs) smoke — submit before Fr NTT, [`finish`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) after MSM; uses [`queue_compute_1`](extern/cuzk/cuzk-vk/src/device.rs) when present; **no readback** on upload. Tests: [`srs_staging_device_local_roundtrip`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs), [`srs_device_local_buffer_download`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs). |
| C.3 | Specialization constants (`log_n`, window, buckets) | ~1.05–1.15× | **B₂ partial:** `vk_oneshot` + [`run_spec_constant_smoke_gpu`](extern/cuzk/cuzk-vk/src/spec_constant_smoke_gpu.rs) + [`run_msm_dispatch_hitcount_smoke`](extern/cuzk/cuzk-vk/src/msm_gpu.rs) (`local_size_x_id = 0` / prebuilt SPV) + [`spec_constant_u32_le`](extern/cuzk/cuzk-vk/src/pipelines.rs) + Fr NTT radix-2 spec tail ([`SPEC_CONSTANTS.md`](extern/cuzk/cuzk-vk/shaders/SPEC_CONSTANTS.md)); **remainder:** further Fr NTT / MSM **`layout(constant_id)`** paths where **naga** glsl-in cannot emit stable spec constants (Pippenger bucket shader uses fixed `G1_PIPP_LOCAL_X` today). |
...
| ID | Lever | Est. vs prior | Notes |
|----|--------|---------------|--------|
| D.1 | Mega-dispatch MSM across *N* circuits | ~1.40–1.80× @ batch≥4 | **B₂ partial (correctness):** batched Pippenger SSBO ([`run_g1_pippenger_msm_gpu_batch`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs)) — variable per-circuit `n`, [`pippenger_plan_batch_tiles`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs) — merges partial window sums across tiles. Host strip sketch [`msm_mega_dense_groups_x`](extern/cuzk/cuzk-vk/src/msm.rs). **Perf remainder:** mega-strip shader tuning + **D.2** SRS window sharing. |
...
- **SRS async read (§C.2 B₁ slice):** [`srs_read_file_spawn`](extern/cuzk/cuzk-vk/src/srs.rs) + **`tests/srs_async_read.rs`** — overlap disk read with the caller thread. **B₂ GPU slice:** [`srs_staging_device_local_upload_submit_async`](extern/cuzk/cuzk-vk/src/srs_staging_gpu.rs) overlaps Fr NTT in [`prove_groth16_partition`](extern/cuzk/cuzk-vk/src/prover.rs) smoke when [`queue_compute_1`](extern/cuzk/cuzk-vk/src/device.rs) exists.
...
| **MSM** | **255-bit** window Pippenger: GPU bucket accumulate ([`g1_pippenger_bucket_acc_tail.comp`](extern/cuzk/cuzk-vk/shaders/g1_pippenger_bucket_acc_tail.comp)) + host Horner ([`g1_msm_bucket`](extern/cuzk/cuzk-vk/src/g1_msm_bucket.rs)); multi-circuit **batch + tiling** ([`run_g1_pippenger_msm_gpu_batch`](extern/cuzk/cuzk-vk/src/g1_pippenger_bucket_gpu.rs)); trunc bit-plane path for small `n` ([`split_msm`](extern/cuzk/cuzk-vk/src/split_msm.rs)). **Open:** A.* perf levers (signed buckets, XYZZ, subgroup), §8.4 mega-strip **perf** + D.2. |
