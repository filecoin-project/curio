...
// Configure the default CUDA memory pool to retain freed allocations
// instead of releasing them back to the OS at every sync point.
// Without this, cudaMallocAsync/cudaFreeAsync provide zero caching
// benefit because the default release threshold is 0.
// Called once per GPU on first use; idempotent.
static void ensure_pool_threshold(int cuda_device_id) {
    static std::atomic<uint32_t> configured_mask{0};  // bit per GPU (up to 32)
    uint32_t bit = 1u << (cuda_device_id & 31);
    if (configured_mask.load(std::memory_order_relaxed) & bit)
        return;

    cudaMemPool_t pool = nullptr;
    if (cudaDeviceGetDefaultMemPool(&pool, cuda_device_id) == cudaSuccess && pool) {
        uint64_t threshold = UINT64_MAX;  // never release cached memory
        cudaMemPoolSetAttribute(pool, cudaMemPoolAttrReleaseThreshold,
                                &threshold);
        fprintf(stderr, "CUZK_TIMING: set pool release threshold to MAX on gpu %d\n",
                cuda_device_id);
    }
        ...
        per_gpu.emplace_back(std::thread([&, tid, n_gpus](size_t num_circuits)
        {
            ...
            try {
                auto t_gpu_start = std::chrono::steady_clock::now();
                {
                    ...
                    fr_t* d_a = get_cached_d_a(gpu, d_a_sz);
