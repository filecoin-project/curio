...
export class MarketClient {
  ...
  async submitPDPv1DealWithUpload(params: {
    ...
    try {
      ...
      // Compute piece_cid from blobs using our utility
      ...
      const deal: Mk20Deal = {
        ...
        identifier: Array.from(uuid.replace(/-/g, '').match(/.{1,2}/g)!.map(hex => parseInt(hex, 16))),
        client,
        data: {
          piece_cid: pieceCid,
          format: { raw: {} },
        ...
        products: {
          pdp_v1: {
            duration: 518400, // Minimum duration
