...
func generate(path, pkg, outpkg, outfile string) error {
		...
		for ifname, methods := range v.Methods {
			...
			for mname, node := range methods {
				...
				if _, ok := info.Methods[mname]; !ok {
					...
					defRes := ""
					if len(results) > 1 {
						// Generate default values for all non-error return values
						var defaults []string
						for i := 0; i < len(results)-1; i++ {
							r := results[i]
							var def string
							switch {
							case r[0] == '*' || r[0] == '<', r == "interface{}", strings.HasPrefix(r, "chan"):
								def = "nil"
							case r == "bool":
								def = "false"
							case r == "string":
								def = `""`
							case r == "int", r == "int64", r == "uint64", r == "uint":
								def = "0"
							default:
								def = "*new(" + r + ")"
							}
	...
	err = doTemplate(&rendered, m, `// Code generated by github.com/filecoin-project/curio/api/gen. DO NOT EDIT.
