...
\restrict UbQAEW9YpAGs4ZQmxk0rCd3VBXDIfa6vf4T0dOTYkouqEX2gg0HqnWKPjr6NhzS
...
CREATE FUNCTION curio.adjust_data_set_refcount_on_update() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION curio.adjust_parked_piece_ref_count_on_update() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION curio.decrement_data_set_refcount() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE pdp_piecerefs
...
CREATE FUNCTION curio.decrement_parked_piece_ref_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE parked_pieces
...
CREATE FUNCTION curio.increment_data_set_refcount() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE pdp_piecerefs
...
CREATE FUNCTION curio.increment_parked_piece_ref_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE parked_pieces
...
CREATE FUNCTION curio.update_pdp_data_set_creates() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION curio.update_pdp_data_set_piece_adds() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    IF OLD.tx_status = 'pending' AND (NEW.tx_status = 'confirmed' OR NEW.tx_status = 'failed') THEN
...
CREATE TABLE curio.harmony_config_history (
    id integer NOT NULL,
    title character varying(300) NOT NULL,
    config text NOT NULL,
...
CREATE SEQUENCE curio.harmony_config_history_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE curio.harmony_config_history_id_seq OWNED BY curio.harmony_config_history.id;
...
ALTER SEQUENCE curio.harmony_config_id_seq OWNED BY curio.harmony_config.id;
...
ALTER SEQUENCE curio.harmony_machine_details_id_seq OWNED BY curio.harmony_machine_details.id;
...
ALTER SEQUENCE curio.harmony_machines_id_seq OWNED BY curio.harmony_machines.id;
...
ALTER SEQUENCE curio.harmony_task_follow_id_seq OWNED BY curio.harmony_task_follow.id;
...
ALTER SEQUENCE curio.harmony_task_history_id_seq OWNED BY curio.harmony_task_history.id;
...
ALTER SEQUENCE curio.harmony_task_id_seq OWNED BY curio.harmony_task.id;
...
ALTER SEQUENCE curio.harmony_task_impl_id_seq OWNED BY curio.harmony_task_impl.id;
...
ALTER SEQUENCE curio.message_sends_eth_send_task_id_seq OWNED BY curio.message_sends_eth.send_task_id;
...
ALTER SEQUENCE curio.parked_piece_refs_ref_id_seq OWNED BY curio.parked_piece_refs.ref_id;
...
ALTER SEQUENCE curio.parked_pieces_id_seq OWNED BY curio.parked_pieces.id;
...
CREATE TABLE curio.pdp_data_set (
    id bigint NOT NULL,
...
CREATE TABLE curio.pdp_data_set_create (
    id text NOT NULL,
    client text NOT NULL,
...
CREATE TABLE curio.pdp_data_set_creates (
    create_message_hash text NOT NULL,
    ok boolean,
...
CREATE TABLE curio.pdp_data_set_delete (
    id text NOT NULL,
    client text NOT NULL,
...
CREATE TABLE curio.pdp_data_set_piece_adds (
    data_set bigint,
    piece text NOT NULL,
...
CREATE TABLE curio.pdp_data_set_pieces (
...
CREATE TABLE curio.pdp_data_sets (
    id bigint NOT NULL,
    prev_challenge_request_epoch bigint,
    challenge_request_task_id bigint,
    challenge_request_msg_hash text,
...
COMMENT ON COLUMN curio.pdp_data_sets.unrecoverable_proving_failure_epoch IS 'Block height at which an unrecoverable proving failure was detected; NULL if active';
...
COMMENT ON COLUMN curio.pdp_data_sets.consecutive_prove_failures IS 'Number of consecutive proving failures (resets on success)';
...
COMMENT ON COLUMN curio.pdp_data_sets.next_prove_attempt_at IS 'Block height before which proving should not be attempted (backoff)';
...
CREATE TABLE curio.pdp_dataset_piece (
...
CREATE TABLE curio.pdp_delete_data_set (
    id bigint NOT NULL,
...
CREATE TABLE curio.pdp_ipni_task (
...
CREATE TABLE curio.pdp_piece_delete (
    id text NOT NULL,
    client text NOT NULL,
    set_id bigint NOT NULL,
...
CREATE TABLE curio.pdp_piece_pull_items (
...
COMMENT ON COLUMN curio.pdp_piece_pull_items.parked_piece_ref IS 'Per-pull-item parked_piece_refs row; preserves the item source URL and is promoted into pdp_piecerefs on success.';
...
COMMENT ON COLUMN curio.pdp_piece_pull_items.pull_parked_piece_id IS 'Set only when PullPiece created the parked_pieces row; ownership marker for retry/expiry cleanup, distinct from parked_piece_ref.piece_id for refs attached to rows owned by other flows.';
...
CREATE TABLE curio.pdp_piece_pulls (
    id bigint NOT NULL,
    service text NOT NULL,
...
CREATE SEQUENCE curio.pdp_piece_pulls_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE curio.pdp_piece_pulls_id_seq OWNED BY curio.pdp_piece_pulls.id;
...
CREATE TABLE curio.pdp_piece_streaming_uploads (
    id uuid NOT NULL,
    service text NOT NULL,
    piece_cid text,
    piece_size bigint,
...
ALTER SEQUENCE curio.pdp_piecerefs_id_seq OWNED BY curio.pdp_piecerefs.id;
...
CREATE TABLE curio.pdp_pipeline (
    created_at timestamp with time zone DEFAULT timezone('UTC'::text, now()) NOT NULL,
    id text NOT NULL,
    client text NOT NULL,
    piece_cid_v2 text NOT NULL,
...
CREATE TABLE curio.pdp_proving_tasks (
    data_set_id bigint NOT NULL,
    task_id bigint NOT NULL
);
...
ALTER SEQUENCE curio.pdp_services_id_seq OWNED BY curio.pdp_services.id;
...
ALTER TABLE ONLY curio.harmony_config ALTER COLUMN id SET DEFAULT nextval('curio.harmony_config_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_config_history ALTER COLUMN id SET DEFAULT nextval('curio.harmony_config_history_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_machine_details ALTER COLUMN id SET DEFAULT nextval('curio.harmony_machine_details_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_machines ALTER COLUMN id SET DEFAULT nextval('curio.harmony_machines_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_task ALTER COLUMN id SET DEFAULT nextval('curio.harmony_task_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_task_follow ALTER COLUMN id SET DEFAULT nextval('curio.harmony_task_follow_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_task_history ALTER COLUMN id SET DEFAULT nextval('curio.harmony_task_history_id_seq'::regclass);
...
ALTER TABLE ONLY curio.harmony_task_impl ALTER COLUMN id SET DEFAULT nextval('curio.harmony_task_impl_id_seq'::regclass);
...
ALTER TABLE ONLY curio.message_sends_eth ALTER COLUMN send_task_id SET DEFAULT nextval('curio.message_sends_eth_send_task_id_seq'::regclass);
...
ALTER TABLE ONLY curio.parked_piece_refs ALTER COLUMN ref_id SET DEFAULT nextval('curio.parked_piece_refs_ref_id_seq'::regclass);
...
ALTER TABLE ONLY curio.parked_pieces ALTER COLUMN id SET DEFAULT nextval('curio.parked_pieces_id_seq'::regclass);
...
ALTER TABLE ONLY curio.pdp_piece_pulls ALTER COLUMN id SET DEFAULT nextval('curio.pdp_piece_pulls_id_seq'::regclass);
...
ALTER TABLE ONLY curio.pdp_piecerefs ALTER COLUMN id SET DEFAULT nextval('curio.pdp_piecerefs_id_seq'::regclass);
...
ALTER TABLE ONLY curio.pdp_services ALTER COLUMN id SET DEFAULT nextval('curio.pdp_services_id_seq'::regclass);
...
ALTER TABLE ONLY curio.eth_keys
...
ALTER TABLE ONLY curio.harmony_config_history
...
ALTER TABLE ONLY curio.harmony_config
...
ALTER TABLE ONLY curio.harmony_machine_details
...
ALTER TABLE ONLY curio.harmony_machines
...
ALTER TABLE ONLY curio.harmony_task_follow
...
ALTER TABLE ONLY curio.harmony_task_history
...
ALTER TABLE ONLY curio.harmony_task_impl
...
ALTER TABLE ONLY curio.harmony_task
...
ALTER TABLE ONLY curio.harmony_task_singletons
...
ALTER TABLE ONLY curio.harmony_test
...
ALTER TABLE ONLY curio.message_send_eth_locks
...
ALTER TABLE ONLY curio.message_sends_eth
...
ALTER TABLE ONLY curio.message_waits_eth
...
ALTER TABLE ONLY curio.parked_piece_refs
...
ALTER TABLE ONLY curio.parked_pieces
...
ALTER TABLE ONLY curio.pdp_data_set
...
ALTER TABLE ONLY curio.pdp_data_set_create
...
ALTER TABLE ONLY curio.pdp_data_set_delete
...
ALTER TABLE ONLY curio.pdp_data_set_piece_adds
...
ALTER TABLE ONLY curio.pdp_data_set_pieces
...
ALTER TABLE ONLY curio.pdp_dataset_piece
...
ALTER TABLE ONLY curio.pdp_delete_data_set
...
ALTER TABLE ONLY curio.pdp_ipni_task
...
ALTER TABLE ONLY curio.pdp_piece_delete
...
ALTER TABLE ONLY curio.pdp_piece_mh_to_commp
...
ALTER TABLE ONLY curio.pdp_piece_pull_items
...
ALTER TABLE ONLY curio.pdp_piece_pulls
...
ALTER TABLE ONLY curio.pdp_piece_streaming_uploads
...
ALTER TABLE ONLY curio.pdp_piece_uploads
...
ALTER TABLE ONLY curio.pdp_piecerefs
...
ALTER TABLE ONLY curio.pdp_pipeline
...
ALTER TABLE ONLY curio.pdp_data_sets
...
ALTER TABLE ONLY curio.pdp_data_set_creates
...
ALTER TABLE ONLY curio.pdp_prove_tasks
...
ALTER TABLE ONLY curio.pdp_proving_tasks
...
ALTER TABLE ONLY curio.pdp_services
...
CREATE INDEX harmony_task_history_recent_task_result_idx ON curio.harmony_task_history USING btree (work_end DESC, name, task_id, result);
...
CREATE INDEX harmony_task_history_task_id_result_index ON curio.harmony_task_history USING btree (task_id, result);
...
CREATE INDEX harmony_task_history_work_index ON curio.harmony_task_history USING btree (work_end DESC, completed_by_host_and_port, name, result);
...
CREATE INDEX idx_config_history_title_time ON curio.harmony_config_history USING btree (title, changed_at DESC);
...
CREATE INDEX idx_harmony_machines_unschedulable ON curio.harmony_machines USING btree (unschedulable);
...
CREATE INDEX idx_harmony_task_unowned_by_name ON curio.harmony_task USING btree (name, update_time) WHERE (owner_id IS NULL);
...
CREATE INDEX idx_message_sends_eth_nonce_lookup ON curio.message_sends_eth USING btree (from_address, nonce DESC) WHERE (send_success = true);
...
CREATE INDEX idx_message_sends_eth_reorg_check ON curio.message_sends_eth USING btree (send_time, send_reason) WHERE ((send_success = true) AND (send_time IS NOT NULL) AND (signed_hash IS NOT NULL));
...
CREATE INDEX idx_message_sends_eth_signed_hash_norm ON curio.message_sends_eth USING btree (lower(TRIM(BOTH FROM signed_hash))) WHERE ((send_success = true) AND (signed_hash IS NOT NULL));
...
CREATE INDEX idx_message_waits_eth_pending ON curio.message_waits_eth USING btree (waiter_machine_id) WHERE ((waiter_machine_id IS NULL) AND (tx_status = 'pending'::text));
...
CREATE INDEX idx_message_waits_eth_reorg_confirmed ON curio.message_waits_eth USING btree (confirmed_block_number, signed_tx_hash) WHERE ((tx_status = 'confirmed'::text) AND (tx_success = true) AND (confirmed_block_number IS NOT NULL));
...
CREATE INDEX idx_message_waits_eth_waiter_pending ON curio.message_waits_eth USING btree (waiter_machine_id, signed_tx_hash) WHERE (tx_status = 'pending'::text);
...
CREATE INDEX idx_parked_piece_refs_piece_id ON curio.parked_piece_refs USING btree (piece_id);
...
CREATE INDEX idx_parked_pieces_cleanup_eligible ON curio.parked_pieces USING btree (ref_count, cleanup_task_id, id);
...
CREATE INDEX idx_parked_pieces_incomplete_fetch ON curio.parked_pieces USING btree (long_term) WHERE ((complete = false) AND (task_id IS NULL));
...
CREATE INDEX idx_pdp_data_set_piece_adds_pdp_pieceref ON curio.pdp_data_set_piece_adds USING btree (pdp_pieceref);
...
CREATE INDEX idx_pdp_data_set_piece_adds_pieces_added ON curio.pdp_data_set_piece_adds USING btree (pieces_added);
...
CREATE INDEX idx_pdp_data_set_piece_adds_unprocessed ON curio.pdp_data_set_piece_adds USING btree (data_set, add_message_hash) WHERE ((add_message_ok = true) AND (pieces_added = false));
...
CREATE INDEX idx_pdp_data_set_pieces_pdp_pieceref ON curio.pdp_data_set_pieces USING btree (pdp_pieceref);
...
CREATE INDEX idx_pdp_data_set_pieces_pending_removal ON curio.pdp_data_set_pieces USING btree (data_set, piece_id) WHERE ((rm_message_hash IS NOT NULL) AND (removed = false));
...
CREATE INDEX idx_pdp_dds_cleanup_task ON curio.pdp_delete_data_set USING btree (cleanup_pieces_task_id);
...
CREATE INDEX idx_pdp_dds_client_term_task ON curio.pdp_delete_data_set USING btree (client_terminate_service_task_id);
...
CREATE INDEX idx_pdp_dds_delete_task ON curio.pdp_delete_data_set USING btree (delete_data_set_task_id);
...
CREATE INDEX idx_pdp_dds_term_task ON curio.pdp_delete_data_set USING btree (terminate_service_task_id);
...
CREATE INDEX idx_pdp_piece_pull_items_parked_piece_ref ON curio.pdp_piece_pull_items USING btree (parked_piece_ref);
...
CREATE INDEX idx_pdp_piece_pull_items_pull_parked_piece_id ON curio.pdp_piece_pull_items USING btree (pull_parked_piece_id);
...
CREATE INDEX idx_pdp_piece_pulls_created_at ON curio.pdp_piece_pulls USING btree (created_at);
...
CREATE INDEX idx_pdp_piece_uploads_notify_pending ON curio.pdp_piece_uploads USING btree (piece_ref) WHERE ((notify_task_id IS NULL) AND (piece_ref IS NOT NULL));
...
CREATE INDEX idx_pdp_piecerefs_indexing_pending ON curio.pdp_piecerefs USING btree (created_at) WHERE ((indexing_task_id IS NULL) AND (needs_indexing = true));
...
CREATE INDEX idx_pdp_piecerefs_ipni_pending ON curio.pdp_piecerefs USING btree (created_at) WHERE ((ipni_task_id IS NULL) AND (needs_ipni = true));
...
CREATE INDEX idx_pdp_piecerefs_zero_refcount_created ON curio.pdp_piecerefs USING btree (created_at, id) WHERE (data_set_refcount = 0);
...
CREATE UNIQUE INDEX machine_details_machine_id ON curio.harmony_machine_details USING btree (machine_id);
...
CREATE UNIQUE INDEX message_sends_eth_success_index ON curio.message_sends_eth USING btree (from_address, nonce) WHERE (send_success IS NOT FALSE);
...
CREATE UNIQUE INDEX parked_pieces_active_piece_key ON curio.parked_pieces USING btree (piece_cid, piece_padded_size, long_term) WHERE (cleanup_task_id IS NULL);
...
CREATE INDEX pdp_piecerefs_indexing_task_id ON curio.pdp_piecerefs USING btree (indexing_task_id) WHERE (indexing_task_id IS NOT NULL);
...
CREATE INDEX pdp_piecerefs_ipni_task_id ON curio.pdp_piecerefs USING btree (ipni_task_id) WHERE (ipni_task_id IS NOT NULL);
...
CREATE INDEX pdp_piecerefs_piece_cid_idx ON curio.pdp_piecerefs USING btree (piece_cid);
...
CREATE TRIGGER parked_piece_refs_delete_refcount AFTER DELETE ON curio.parked_piece_refs FOR EACH ROW EXECUTE FUNCTION curio.decrement_parked_piece_ref_count();
...
CREATE TRIGGER parked_piece_refs_insert_refcount AFTER INSERT ON curio.parked_piece_refs FOR EACH ROW EXECUTE FUNCTION curio.increment_parked_piece_ref_count();
...
CREATE TRIGGER parked_piece_refs_update_refcount AFTER UPDATE ON curio.parked_piece_refs FOR EACH ROW EXECUTE FUNCTION curio.adjust_parked_piece_ref_count_on_update();
...
CREATE TRIGGER pdp_data_set_add_message_status_change AFTER UPDATE OF tx_status, tx_success ON curio.message_waits_eth FOR EACH ROW EXECUTE FUNCTION curio.update_pdp_data_set_piece_adds();
...
CREATE TRIGGER pdp_data_set_create_message_status_change AFTER UPDATE OF tx_status, tx_success ON curio.message_waits_eth FOR EACH ROW EXECUTE FUNCTION curio.update_pdp_data_set_creates();
...
CREATE TRIGGER pdp_data_set_piece_delete AFTER DELETE ON curio.pdp_data_set_pieces FOR EACH ROW WHEN ((old.pdp_pieceref IS NOT NULL)) EXECUTE FUNCTION curio.decrement_data_set_refcount();
...
CREATE TRIGGER pdp_data_set_piece_insert AFTER INSERT ON curio.pdp_data_set_pieces FOR EACH ROW WHEN ((new.pdp_pieceref IS NOT NULL)) EXECUTE FUNCTION curio.increment_data_set_refcount();
...
CREATE TRIGGER pdp_data_set_piece_update AFTER UPDATE ON curio.pdp_data_set_pieces FOR EACH ROW EXECUTE FUNCTION curio.adjust_data_set_refcount_on_update();
...
ALTER TABLE ONLY curio.harmony_machine_details
    ADD CONSTRAINT harmony_machine_details_machine_id_fkey FOREIGN KEY (machine_id) REFERENCES curio.harmony_machines(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.harmony_task_follow
    ADD CONSTRAINT harmony_task_follow_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES curio.harmony_machines(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.harmony_task_impl
    ADD CONSTRAINT harmony_task_impl_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES curio.harmony_machines(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.harmony_task
    ADD CONSTRAINT harmony_task_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES curio.harmony_machines(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.harmony_task_singletons
    ADD CONSTRAINT harmony_task_singletons_task_id_fkey FOREIGN KEY (task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.message_waits_eth
    ADD CONSTRAINT message_waits_eth_waiter_machine_id_fkey FOREIGN KEY (waiter_machine_id) REFERENCES curio.harmony_machines(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.parked_piece_refs
    ADD CONSTRAINT parked_piece_refs_piece_id_fkey FOREIGN KEY (piece_id) REFERENCES curio.parked_pieces(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set
    ADD CONSTRAINT pdp_data_set_challenge_request_task_id_fkey FOREIGN KEY (challenge_request_task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_data_set_piece_adds
    ADD CONSTRAINT pdp_data_set_piece_adds_pdp_pieceref_fkey FOREIGN KEY (pdp_pieceref) REFERENCES curio.pdp_piecerefs(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set_pieces
    ADD CONSTRAINT pdp_data_set_pieces_pdp_pieceref_fkey FOREIGN KEY (pdp_pieceref) REFERENCES curio.pdp_piecerefs(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_delete_data_set
    ADD CONSTRAINT pdp_delete_data_set_cleanup_task_fk FOREIGN KEY (cleanup_pieces_task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_delete_data_set
    ADD CONSTRAINT pdp_delete_data_set_client_terminate_task_fk FOREIGN KEY (client_terminate_service_task_id) REFERENCES curio.harmony_task(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_delete_data_set
    ADD CONSTRAINT pdp_delete_data_set_delete_task_fk FOREIGN KEY (delete_data_set_task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_delete_data_set
    ADD CONSTRAINT pdp_delete_data_set_terminate_task_fk FOREIGN KEY (terminate_service_task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_piece_pull_items
    ADD CONSTRAINT pdp_piece_pull_items_fetch_id_fkey FOREIGN KEY (fetch_id) REFERENCES curio.pdp_piece_pulls(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_piece_pull_items
    ADD CONSTRAINT pdp_piece_pull_items_parked_piece_ref_fkey FOREIGN KEY (parked_piece_ref) REFERENCES curio.parked_piece_refs(ref_id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_piece_pull_items
    ADD CONSTRAINT pdp_piece_pull_items_pull_parked_piece_id_fkey FOREIGN KEY (pull_parked_piece_id) REFERENCES curio.parked_pieces(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_piece_pull_items
    ADD CONSTRAINT pdp_piece_pull_items_task_id_fkey FOREIGN KEY (task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_piece_pulls
    ADD CONSTRAINT pdp_piece_pulls_service_fkey FOREIGN KEY (service) REFERENCES curio.pdp_services(service_label) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_piece_uploads
    ADD CONSTRAINT pdp_piece_uploads_piece_ref_fkey FOREIGN KEY (piece_ref) REFERENCES curio.parked_piece_refs(ref_id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_piece_uploads
    ADD CONSTRAINT pdp_piece_uploads_service_fkey FOREIGN KEY (service) REFERENCES curio.pdp_services(service_label) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_piecerefs
    ADD CONSTRAINT pdp_piecerefs_piece_ref_fkey FOREIGN KEY (piece_ref) REFERENCES curio.parked_piece_refs(ref_id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_piecerefs
    ADD CONSTRAINT pdp_piecerefs_service_fkey FOREIGN KEY (service) REFERENCES curio.pdp_services(service_label) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_sets
    ADD CONSTRAINT pdp_proof_sets_challenge_request_task_id_fkey FOREIGN KEY (challenge_request_task_id) REFERENCES curio.harmony_task(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY curio.pdp_data_sets
    ADD CONSTRAINT pdp_proof_sets_service_fkey FOREIGN KEY (service) REFERENCES curio.pdp_services(service_label) ON DELETE RESTRICT;
...
ALTER TABLE ONLY curio.pdp_data_set_creates
    ADD CONSTRAINT pdp_proofset_creates_create_message_hash_fkey FOREIGN KEY (create_message_hash) REFERENCES curio.message_waits_eth(signed_tx_hash) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set_creates
    ADD CONSTRAINT pdp_proofset_creates_service_fkey FOREIGN KEY (service) REFERENCES curio.pdp_services(service_label) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set_piece_adds
    ADD CONSTRAINT pdp_proofset_root_adds_add_message_hash_fkey FOREIGN KEY (add_message_hash) REFERENCES curio.message_waits_eth(signed_tx_hash) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set_piece_adds
    ADD CONSTRAINT pdp_proofset_root_adds_proofset_fkey FOREIGN KEY (data_set) REFERENCES curio.pdp_data_sets(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set_pieces
    ADD CONSTRAINT pdp_proofset_roots_add_message_hash_fkey FOREIGN KEY (add_message_hash) REFERENCES curio.message_waits_eth(signed_tx_hash) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_data_set_pieces
    ADD CONSTRAINT pdp_proofset_roots_proofset_fkey FOREIGN KEY (data_set) REFERENCES curio.pdp_data_sets(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_prove_tasks
    ADD CONSTRAINT pdp_prove_tasks_proofset_fkey FOREIGN KEY (data_set) REFERENCES curio.pdp_data_sets(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_prove_tasks
    ADD CONSTRAINT pdp_prove_tasks_task_id_fkey FOREIGN KEY (task_id) REFERENCES curio.harmony_task(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_proving_tasks
    ADD CONSTRAINT pdp_proving_tasks_data_set_id_fkey FOREIGN KEY (data_set_id) REFERENCES curio.pdp_data_set(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY curio.pdp_proving_tasks
    ADD CONSTRAINT pdp_proving_tasks_task_id_fkey FOREIGN KEY (task_id) REFERENCES curio.harmony_task(id) ON DELETE CASCADE;
...
\unrestrict UbQAEW9YpAGs4ZQmxk0rCd3VBXDIfa6vf4T0dOTYkouqEX2gg0HqnWKPjr6NhzS
