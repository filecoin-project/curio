...
func TestParkPieceCanAccept_SliceBounds(t *testing.T) {
	...
	engine, err := harmonytask.New(db, []harmonytask.TaskInterface{ppt}, "testhost:1234", noopPeerConnector{}, ffigpu.Inspector{})
	require.NoError(t, err)
...
func TestParkedPieceRefCount_TriggersMaintainCount(t *testing.T) {
	...
	require.Equal(t, 0, parkedPieceRefCount(t, ctx, db, pieceID))

	ref1 := insertParkedPieceRefcountRef(t, ctx, db, pieceID)
	require.Equal(t, 1, parkedPieceRefCount(t, ctx, db, pieceID))

	ref2 := insertParkedPieceRefcountRef(t, ctx, db, pieceID)
	require.Equal(t, 2, parkedPieceRefCount(t, ctx, db, pieceID))

	deleteParkedPieceRefRow(t, ctx, db, ref1)
	require.Equal(t, 1, parkedPieceRefCount(t, ctx, db, pieceID))

	deleteParkedPieceRefRow(t, ctx, db, ref2)
	require.Equal(t, 0, parkedPieceRefCount(t, ctx, db, pieceID))
}
...
func TestParkedPieceRefCount_UpdateTriggerMovesCount(t *testing.T) {
	...
	require.Equal(t, 2, parkedPieceRefCount(t, ctx, db, srcID))
	require.Equal(t, 0, parkedPieceRefCount(t, ctx, db, dstID))

	_, err = db.Exec(ctx, `UPDATE parked_piece_refs SET piece_id = $1 WHERE ref_id = ANY($2::bigint[])`, dstID, []int64{refA, refB})
	require.NoError(t, err)

	require.Equal(t, 0, parkedPieceRefCount(t, ctx, db, srcID))
	require.Equal(t, 2, parkedPieceRefCount(t, ctx, db, dstID))
...
func TestParkedPieceRefCount_CleanupCandidateVisibility(t *testing.T) {
	...
	deleteParkedPieceRefRow(t, ctx, db, refID)
...
func TestParkedPieceRefCount_CascadeDeleteLeavesNoOrphans(t *testing.T) {
	...
	require.Equal(t, 2, parkedPieceRefCount(t, ctx, db, doomedID))
	require.Equal(t, 1, parkedPieceRefCount(t, ctx, db, survivorID))
...
func TestParkedPieceRefCount_ConcurrentInsertsConverge(t *testing.T) {
	...
	require.Equal(t, n, parkedPieceRefCount(t, ctx, db, pieceID))
}
...
func deleteParkedPieceRefRow(t *testing.T, ctx context.Context, db *harmonydb.DB, refID int64) {
	t.Helper()
	_, err := db.Exec(ctx, `DELETE FROM parked_piece_refs WHERE ref_id = $1`, refID)
	require.NoError(t, err)
}

func parkedPieceRefCount(t *testing.T, ctx context.Context, db *harmonydb.DB, pieceID int64) int {
	t.Helper()
	var n int
