...
func StartTasks(ctx context.Context, dependencies *deps.Deps, shutdownChan chan struct{}) (*harmonytask.TaskEngine, error) {
...

	{
		...
		if cfg.Subsystems.EnablePDP {
			...
			pdpv1.NewPDPv1DataSetDeleteWatcher(db, chainSched, ethClient)
			pdpv1.NewPDPv1WatcherPieceDelete(db, chainSched, ethClient)
			pdpv1AddDataSet := pdpv1.NewPDPTaskAddDataSet(db, es, ethClient, full)
			pdpv1AddPiece := pdpv1.NewPDPTaskAddPiece(db, es, ethClient)
			pdpv1DelDataSetTask := pdpv1.NewPDPTaskDeleteDataSet(db, es, ethClient)
