...
impl Boolean {
        ...
        match (a, b) {
            ...
            (&Boolean::Constant(true), a) | (a, &Boolean::Constant(true)) => {
                cs.enforce(
                    ...
                    |lc| {
                        let lc = lc + CS::one();
                        // Subtract a's contribution: equivalent to `- &a.lc(one, ONE)`
                        // but without allocating a temporary LC.
                        match *a {
                            Boolean::Constant(true) => lc - (Scalar::ONE, CS::one()),
                            Boolean::Constant(false) => lc,
                            Boolean::Is(ref v) => lc - (Scalar::ONE, v.get_variable()),
                            Boolean::Not(ref v) => {
                                lc - (Scalar::ONE, CS::one()) + (Scalar::ONE, v.get_variable())
                            }
                        }
                    },
                );
            ...
            (&Boolean::Constant(false), a) | (a, &Boolean::Constant(false)) => {
                cs.enforce(
                    ...
                    |lc| lc,
                    |lc| lc,
                    |lc| a.add_to_lc(lc, CS::one(), Scalar::ONE),
                );
            ...
            (a, b) => {
                cs.enforce(
                    ...
                    |lc| lc,
                    |lc| {
                        let lc = a.add_to_lc(lc, CS::one(), Scalar::ONE);
                        // Subtract b's contribution inline
                        match *b {
                            Boolean::Constant(true) => lc - (Scalar::ONE, CS::one()),
                            Boolean::Constant(false) => lc,
                            Boolean::Is(ref v) => lc - (Scalar::ONE, v.get_variable()),
    ...
    /// Add this boolean's linear combination terms directly into an existing
    /// `LinearCombination`, avoiding a temporary allocation.
    ///
    /// Equivalent to `lc = lc + &self.lc(one, coeff)` but without creating
    /// and immediately dropping a temporary `LinearCombination`. This
    /// eliminates one Vec allocation per call — critical in tight loops like
    /// `UInt32::addmany` which calls this 32× per operand.
    #[inline]
    pub fn add_to_lc<Scalar: PrimeField>(
        &self,
        ...
        match *self {
            ...
            Boolean::Is(ref v) => lc + (coeff, v.get_variable()),
            Boolean::Not(ref v) => lc + (coeff, one) - (coeff, v.get_variable()),
        }
    }

    /// Subtract this boolean's linear combination terms from an existing
    /// `LinearCombination`, avoiding a temporary allocation.
    ///
    /// Equivalent to `lc = lc - &self.lc(one, coeff)` but without creating
    /// a temporary `LinearCombination`.
    #[inline]
    pub fn sub_from_lc<Scalar: PrimeField>(
        &self,
        lc: LinearCombination<Scalar>,
        one: Variable,
        coeff: Scalar,
        ...
        CS: ConstraintSystem<Scalar>,
    {
        ...
        cs.enforce(
            || "ch computation",
            |lc| {
                let lc = b.add_to_lc(lc, CS::one(), Scalar::ONE);
                // Subtract c's contribution inline
                match *c {
                    Boolean::Constant(true) => lc - (Scalar::ONE, CS::one()),
                    Boolean::Constant(false) => lc,
                    Boolean::Is(ref v) => lc - (Scalar::ONE, v.get_variable()),
        ...
        CS: ConstraintSystem<Scalar>,
    {
        ...
        cs.enforce(
            ...
            |lc| {
                ...
                let lc = bc.add_to_lc(lc, CS::one(), two);
                // subtract b
                let lc = match *b {
                    Boolean::Constant(true) => lc - (Scalar::ONE, CS::one()),
                    Boolean::Constant(false) => lc,
            ...
            |lc| a.add_to_lc(lc, CS::one(), Scalar::ONE),
            |lc| bc.add_to_lc(lc, CS::one(), Scalar::ONE) - maj,
        );
