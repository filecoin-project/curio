//! Fp2 mul on GPU vs CPU.

use blstrs::Fp2;
use cuzk_vk::device::VulkanDevice;
use cuzk_vk::fp2_gpu::run_fp2_mul_mod_gpu;
use ff::Field;
...
fn fp2_mul_gpu_matches_cpu() {
    ...
    for _ in 0..32 {
        ...
        let want = a * b;
        let got = run_fp2_mul_mod_gpu(&dev, &a, &b).expect("gpu fp2 mul");
        assert_eq!(got, want);
    }
    let z = Fp2::ZERO;
    let o = Fp2::ONE;
    assert_eq!(run_fp2_mul_mod_gpu(&dev, &z, &o).unwrap(), z);
    assert_eq!(run_fp2_mul_mod_gpu(&dev, &o, &o).unwrap(), o);
}
