...
func (t *WinPostTask) Do(taskID harmonytask.TaskID, stillOwned func() bool) (done bool, err error) {
	...
	// winner check
	{
		...
		eproof, err = isRoundWinner(ctx, round, maddr, rbase, mbi, t.api)
		if err != nil {
...
func (t *WinPostTask) computeTicket(ctx context.Context, maddr address.Address, brand *types.BeaconEntry, round abi.ChainEpoch, chainRand *types.Ticket, mbi *api.MiningBaseInfo) (*types.Ticket, error) {
	...
	vrfOut, err := computeVRF(ctx, t.api.WalletSign, mbi.WorkerKey, input)
	if err != nil {
		return nil, err
	}
...
func computeVRF(ctx context.Context, sign signFunc, worker address.Address, sigInput []byte) ([]byte, error) {
	...
	if sig.Type != crypto.SigTypeBLS {
		return nil, xerrors.Errorf("miner worker address was not a BLS key")
	}
...
func isRoundWinner(ctx context.Context, round abi.ChainEpoch,
	...
	electionRand, err := lrand.DrawRandomnessFromBase(brand.Data, crypto.DomainSeparationTag_ElectionProofProduction, round, buf.Bytes())
	if err != nil {
