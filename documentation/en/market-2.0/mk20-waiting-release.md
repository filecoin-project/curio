...
insert after download/reference writes.

## Evidence Boundary and Finite Cost Check

Record the exact source revision, engine version, isolation settings, and
observed contention with each SQL run. Unit tests and compilation do not prove
database row effects; normal sealing is not proof of every aggregate, rollback,
contention, or completion scenario. Results from an older revision are historical
evidence, not execution on a new integration. An unavailable attributable retry
count is NOT MEASURED, not zero. Isolated fixture timings are not production
