...
func (sb *SealCalls) WriteUploadPiece(ctx context.Context, pieceID storiface.PieceNumber, size int64, data io.Reader, storageType storiface.PathType) (abi.PieceInfo, error) {
	// Use storageType in AcquireSector
	...
	destFile, err := os.OpenFile(tempDest, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		return abi.PieceInfo{}, xerrors.Errorf("creating temp piece file '%s': %w", tempDest, err)
	}
	...
	if err != nil {
		_ = destFile.Close()
		return abi.PieceInfo{}, xerrors.Errorf("copying piece data: %w", err)
	}

	if err := destFile.Close(); err != nil {
		return abi.PieceInfo{}, xerrors.Errorf("closing temp piece file: %w", err)
	}

	if n != size {
		return abi.PieceInfo{}, xerrors.Errorf("short write: %d", n)
	}

	digest, pieceSize, err := wr.Digest()
	if err != nil {
		return abi.PieceInfo{}, xerrors.Errorf("computing piece digest: %w", err)
	}

	pcid, err := commcid.DataCommitmentV1ToCID(digest)
	if err != nil {
		return abi.PieceInfo{}, xerrors.Errorf("computing piece CID: %w", err)
	}
	...
	if err := os.Rename(tempDest, dest); err != nil {
		return abi.PieceInfo{}, xerrors.Errorf("rename temp piece to dest %s -> %s: %w", tempDest, dest, err)
	}

	removeTemp = false
	return abi.PieceInfo{PieceCID: pcid, Size: psize}, nil
}
