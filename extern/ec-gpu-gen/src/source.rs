...
fn const_field_glsl_uint(name: &str, limbs: &[Limb32]) -> String {
    let n = limbs.len();
    let vals = limbs
        .iter()
        .map(|l| format!("{}u", l.value()))
        .collect::<Vec<_>>()
        .join(", ");
    format!("const uint {name}[{n}] = uint[]({vals});")
}

/// Like [`params`], but emits GLSL 450 `uint` limb tables instead of OpenCL `CONSTANT FIELD` syntax.
///
/// Tokens use the `FIELD` prefix; call [`glsl_32_bit_field_params`] to substitute `F::name()` (see
/// `ec_gpu::GpuName`) for `FIELD`, matching the OpenCL generator.
fn params_glsl<F: GpuField>() -> String {
    let one = Limb32::one_limbs::<F>();
    let p = Limb32::modulus_limbs::<F>();
    let r2 = Limb32::calculate_r2::<F>();
    let limbs = one.len();
    let inv = Limb32::calc_inv(p[0]);
    let zero = vec![Limb32::zero(); limbs];
    let limb_def = "#define FIELD_limb uint".to_string();
    let limbs_def = format!("#define FIELD_LIMBS {}", limbs);
    let limb_bits_def = "#define FIELD_LIMB_BITS 32".to_string();
    let inv_def = format!("#define FIELD_INV {}u", inv.value());
    let typedef = "struct FIELD {\n    FIELD_limb val[FIELD_LIMBS];\n};".to_string();
    let p_def = const_field_glsl_uint("FIELD_P", &p);
    let r2_def = const_field_glsl_uint("FIELD_R2", &r2);
    let one_def = const_field_glsl_uint("FIELD_ONE", &one);
    let zero_def = const_field_glsl_uint("FIELD_ZERO", &zero);
    let header = "#version 450\n// Scalar field parameters (32-bit Montgomery limbs). Generated by ec-gpu-gen.\n\n";
    [
        header.to_string(),
        limb_def,
        limbs_def,
...
/// GLSL 450 preamble for a 32-bit-limb prime field: `#define`s, `struct`, and `const uint` limb tables.
///
/// This is the Vulkan-oriented counterpart to the OpenCL `FIELD` block in [`params`]; it does not
/// include `field.cl` arithmetic yet.
pub fn glsl_32_bit_field_params<F: GpuField>() -> String {
    params_glsl::<F>().replace("FIELD", &F::name())
}
