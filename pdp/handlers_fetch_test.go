...
type mockFetchStore struct {
	...
	fetchItemStatuses map[string]*FetchItemStatus // keyed by v1 CID string (from pdp_piece_fetch_items)
	fetchPieces       []FetchPiece                // v1 CID + raw size + source_url
	createError       error
	...
	createdPieces              []FetchPiece // v1 CID + raw size + source_url
	getStatusesCalled          bool
	getFetchItemStatusesCalled bool
...
func (m *mockFetchStore) GetFetchItemStatuses(ctx context.Context, fetchID int64, pieceCids []cid.Cid) (map[string]*FetchItemStatus, error) {
	m.getFetchItemStatusesCalled = true
	if m.fetchItemStatuses == nil {
		return make(map[string]*FetchItemStatus), nil
	}
	return m.fetchItemStatuses, nil
}
