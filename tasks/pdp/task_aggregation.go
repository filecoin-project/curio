...
func (a *AggregatePDPDealTask) Do(taskID harmonytask.TaskID, stillOwned func() bool) (done bool, err error) {
	...
	for _, piece := range pieces {
		...
		if piece.Aggregation != int(mk20.AggregateTypeV1) && piece.Aggregation != int(mk20.AggregateTypeV2) {
			return false, xerrors.Errorf("unsupported aggregation type %d for piece %s deal %s task %d", piece.Aggregation, piece.PieceCidV2, piece.ID, taskID)
		}
		...
		rawSizes = append(rawSizes, int64(pinfo.RawSize))
