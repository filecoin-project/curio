...
import (
	...
	"github.com/filecoin-project/curio/web/api/webrpctypes"
)

func (a *PoRep) PipelineStatsSnap(ctx context.Context) (*webrpctypes.PipelineStats, error) {
	var out webrpctypes.PipelineStats
...
`
	...
	if len(cts) == 0 {
		// No active pipelines
		return &webrpctypes.PipelineStats{
			Total: 0,
			Stages: []webrpctypes.PipelineStage{
	...
	out.Stages = []webrpctypes.PipelineStage{
...
func (a *PoRep) PipelineStatsSDR(ctx context.Context) (*webrpctypes.PipelineStats, error) {
	var out webrpctypes.PipelineStats
