...
The task sends a separate `settleRail` transaction for each selected rail and atomically records its hash in `message_waits_eth` and `filecoin_payment_transactions`. Later passes skip rails that still appear in `filecoin_payment_transactions`, until the watcher finishes processing those transactions and removes their tracking rows. This prevents another settlement from being queued while the previous attempt is outstanding. Each out-of-gas estimate reduces the remaining epoch range to 75% of its previous size, rounded down, before trying again. If gas estimation requires a smaller settlement range than the resolver's target, the tracking row has `retry = TRUE` so the watcher can request another pass after confirmation.
