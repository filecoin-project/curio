...
// FetchPiecePollInterval is how often to poll for new fetch items
var FetchPiecePollInterval = time.Second

// PDPFetchPieceTask downloads pieces from external SPs, verifies CommP,
// and stores them in custore:// for StorePiece to pick up.
//
// Flow:
//  1. Handler creates pdp_piece_fetch_items with source_url
...
// NewPDPFetchPieceTask creates a new PDPFetchPieceTask
func NewPDPFetchPieceTask(ctx context.Context, db *harmonydb.DB, storage paths.StashStore, max int) *PDPFetchPieceTask {
	t := &PDPFetchPieceTask{
		db:      db,
		storage: storage,
		max:     max,
	...
	go t.pollFetchItems(ctx)
	return t
}

// pollFetchItems polls for fetch items that need processing
func (t *PDPFetchPieceTask) pollFetchItems(ctx context.Context) {
	ticker := time.NewTicker(FetchPiecePollInterval)
	defer ticker.Stop()
	...
	for {
		...
		// 3. Do NOT already have a parked_pieces entry (fetch already completed)
		...
		`)
		if err != nil {
			log.Errorf("failed to query fetch items: %s", err)
			select {
			case <-ctx.Done():
				return
			case <-ticker.C:
		...
		for _, item := range items {
			...
			t.TF.Val(ctx)(func(id harmonytask.TaskID, tx *harmonydb.Tx) (shouldCommit bool, err error) {
				...
				n, err := tx.Exec(`
					UPDATE pdp_piece_fetch_items fi
					SET task_id = $1
				...
				`, id, fetchID, pieceCid)
				if err != nil {
					return false, xerrors.Errorf("updating fetch item task_id: %w", err)
				}
...
// fetchItemData holds the data for a fetch item task
...
func (t *PDPFetchPieceTask) Do(taskID harmonytask.TaskID, stillOwned func() bool) (done bool, err error) {
	...
	if len(items) == 0 {
		return false, xerrors.Errorf("no fetch item found for task_id: %d", taskID)
	}
	...
	if err != nil {
		...
		log.Errorw("fetch piece failed", "error", err, "pieceCid", item.PieceCid, "sourceUrl", item.SourceURL)
	...
	_, err = t.db.BeginTransaction(ctx, func(tx *harmonydb.Tx) (bool, error) {
		...
		// Clear task_id from fetch item (task is done)
		_, err = tx.Exec(`
		...
		if err != nil {
			return false, xerrors.Errorf("clearing fetch item task_id: %w", err)
		}
	...
	log.Infow("fetch piece complete", "pieceCid", item.PieceCid, "custoreUrl", custoreURL)
	return true, nil
}
...
func (t *PDPFetchPieceTask) downloadAndVerify(ctx context.Context, sourceURL string, expectedSize int64, expectedCid cid.Cid) (uuid.UUID, error) {
	...
	// 1 hour timeout for entire fetch operation
...
func (t *PDPFetchPieceTask) CanAccept(ids []harmonytask.TaskID, engine *harmonytask.TaskEngine) (*harmonytask.TaskID, error) {
	id := ids[0]
	return &id, nil
}

func (t *PDPFetchPieceTask) TypeDetails() harmonytask.TaskTypeDetails {
	return harmonytask.TaskTypeDetails{
		Name: "PDPv0_FetchPiece",
...
func (t *PDPFetchPieceTask) Adder(taskFunc harmonytask.AddTaskFunc) {
	t.TF.Set(taskFunc)
}

var _ harmonytask.TaskInterface = &PDPFetchPieceTask{}
var _ = harmonytask.Reg(&PDPFetchPieceTask{})
